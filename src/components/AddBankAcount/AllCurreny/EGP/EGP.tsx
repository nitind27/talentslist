...
// import EGPIban from "./EGPIban";
// import { ICurrency } from "@/api/payment/types";

// const EGP = ({ currencyData }: { currencyData: ICurrency }) => {
//   return (
//     <>
//       <div className="row">
...
//             <EGPIban key={index}  currencyData={formdata} />
//           ))}
//         </div>
//       </div>
//     </>
...
function EGP({ currencyData }: { currencyData: ICurrency }) {
  const initialValues = {
    account_holder_name: "",
    address: "",
    postCode: "",
  ...
  const validationSchema = Yup.object({
    ...
    BankcodeBICSWIFT: Yup.string()
          .min(8,"At list 8 letter require").max(11,"You can write only up to 11 letters")
            .matches(/^[A-Za-z]{6}[A-Za-z\d]{2}([A-Za-z\d]{3})?$/, "Invalid BIC/SWIFT code")
            .required("BIC/SWIFT code is required *"),
          IBAN: Yup.string()
            .matches(/^[a-zA-Z]{2}[a-zA-Z0-9 ]{12,40}$/, "Invalid IBAN")
            .min(14,"At list 14 letter require").max(42,"You can write only up to 42 letters")
      ...
      {(formik) => (
        <Form>
          <div className="row">
            ...
            <div className="col-6">
              {currencyData.data.form.map((formdata, index) => (
                <EGPIban key={index} currencyData={formdata} formik={formik} />
              ))}
            </div>
